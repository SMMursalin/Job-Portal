    ...
    if request.method == 'POST':
        ...
        if user:
            try:
                ...
                if user1.type == "recruiter" and user1.status != "pending" and user1.status != "Reject":
                    login(request, user)
                    error = "no"
                else:
                    error = "not"
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            StudentUser.objects.create(
                user=user, mobile=con, image=i, gender=gen, type="student")
            error = "no"
        except:
            error = "yes"
    d = {'error': error}
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            Recruiter.objects.create(user=user, mobile=con, image=i, gender=gen,
                                     company=company, type="recruiter", status="pending")
            error = "no"
        except:
            error = "yes"
    d = {'error': error}
...
def delete_user(request, pid):
    ...
    student.delete()
    return redirect("view_users.html")
...
def delete_recruiter(request,pid):
    ...
    recruiter.delete()
    return redirect("recruiter_all.html")
